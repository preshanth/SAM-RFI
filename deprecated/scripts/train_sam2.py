#!/usr/bin/env python3
"""
Comprehensive SAM2 Training Script with Full Experiment Tracking
...
Usage:
    ...
    python scripts/train_sam2.py --config configs/experiments/exp1_synthetic.yaml --resume output/exp1_synthetic/checkpoint_epoch5.pth
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train SAM2 with full experiment tracking")
    ...
    print(f"\n{'='*60}")
    print(f"SAM2 Training - Experiment: {config['experiment']['name']}")
    print(f"{'='*60}\n")
    ...
    model_name = checkpoint_map[config['model']['checkpoint']]

    tracker.log(f"Loading SAM2 model: {model_name}")
    ...
    if config['model'].get('freeze_encoders', True):
        ...
        tracker.log("  Frozen vision and prompt encoders (training mask decoder only)")
