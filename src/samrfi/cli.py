...
def create_parser() -> argparse.ArgumentParser:
    ...
    process_parser = subparsers.add_parser(
        "process",
        help="Process measurement set for RFI detection"
    )
    ...
    process_parser.add_argument(
        ...
        type=int,
        default=0,
        help="Antenna ID to process (default: 0)"
    )
    process_parser.add_argument(
        "--model",
        default="sam2",
        help="SAM model version to use (default: sam2)"
    )
    
    # Info command
    info_parser = subparsers.add_parser(
        "info",
        help="Show package and system information"
    )
...
def cmd_info(args: argparse.Namespace) -> int:
    ...
    try:
        import casacore
        print(f"python-casacore: {casacore.__version__}")
    except ImportError:
