...
class RFIDatasetCreator:
    ...
    def __init__(self, output_dir: str = "rfi_dataset"):
        ...
        if not HF_AVAILABLE:
            raise ImportError("HuggingFace dependencies not available. Please install datasets and huggingface-hub.")
    ...
    def create_training_dataset(self, 
        ...
        logger.info(f"Created dataset: {len(train_data)} train, {len(val_data)} validation samples")
        ...
        # Save metadata
        self._save_dataset_metadata(dataset_dict, {
    ...
    def _generate_synthetic_data(self, num_samples: int, image_size: int) -> List[Dict[str, Any]]:
    ...
    def _create_synthetic_sample(self, image_size: int) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    ...
    def _save_dataset_metadata(self, dataset: DatasetDict, metadata: Dict[str, Any]) -> None:
    ...
    def _compute_dataset_stats(self, dataset: Dataset) -> Dict[str, Any]:
        ...
        return {
            ...
            'rfi_type_distribution': {rfi_type: rfi_types.count(rfi_type) for rfi_type in set(rfi_types)},
    ...
    def push_to_hub(self, 
        ...
        if commit_message is None:
            commit_message = f"Add SAM-RFI training dataset ({datetime.now().strftime('%Y-%m-%d')})"
