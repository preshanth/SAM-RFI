...
class SamRFI:
    ...
    def add_rifi_spectrograph(
        self, amplitude, center_freq, bandwidth, horizontal=False, table=False
    ):

        ## note to self, make sure to add time to the table
        if horizontal:
            self.temp_spectrograph = np.apply_along_axis(
                self.add_rfi,
                axis=0,
                arr=self.temp_spectrograph,
                amplitude=amplitude,
    ...
    def add_square_rifi_spectrograph(
        self, amplitude, center_freq, bandwidth, horizontal=False, table=False
    ):
    ...
    def create_model_spectrograph(
        ...
        for index, row in self.rfi_table.iterrows():
            ...
            if row["rfi_type"] == "persistent_sq":
                self.add_square_rifi_spectrograph(
                    row["amplitude"], row["center_freq"], row["bandwidth"]
                )

        self.model_spectrograph = self.temp_spectrograph

    def intermittent_rfi(
        self, amplitude, center_freq, bandwidth, time_period, duty_cycle, time_offset=0, table=False
    ):
    ...
    def generate_rfi_waterfall(
        ...
        for i in np.arange(num_persistent):
            self.add_rifi_spectrograph(
                ...
                center_freq=np.random.uniform(min_freq + edge_buffer, max_freq - edge_buffer),
                bandwidth=np.abs(np.random.normal(11, 5)),
                table=True,
            )
    ...
    def find_spectrograph_peaks(self, min_distance=10, threshold_abs=30):
        ...
        """
        max_peaks = peak_local_max(
            self.spectrograph, min_distance=min_distance, threshold_abs=threshold_abs
        )
    ...
    def create_patches(self, image, patch_size=256):
        ...
        padded_image = np.pad(
            image, ((0, pad_rows), (0, pad_cols)), mode="constant", constant_values=0
        )
        ...
        if not patch_run:
            ...
            for baseline in tqdm(range(self.rfi_antenna_data.shape[0])):
                ...
                for pol in range(self.rfi_antenna_data.shape[1]):
                    ...
                    single_data = np.clip(
                        single_data, (median + (stat * 1)), (median + (stat * 10))
                    )
                    ...
                    single_data = np.pad(
                        ...
                        pad_width=((pad_width, pad_width), (pad_width, pad_width)),
                        mode="constant",
                        constant_values=(median * 0.5 + (stat * 1)),
                    )
                ...
                for pol in range(self.rfi_antenna_data.shape[1]):
                    ...
                    for patch in patches:
                        ...
                        inputs = self.processor(
                            single_patch, input_boxes=[[bbox]], return_tensors="pt"
                        )
            ...
            for j in range(self.rfi_antenna_data.shape[1]):
                ...
                if save:
                    ...
                    norm1 = ImageNormalize(
                        self.rfi_antenna_data[i, j, :, :].T, interval=ZScaleInterval()
                    )
                    norm2 = ImageNormalize(self.flags[i, j, :, :].T, interval=ZScaleInterval())

                    residual = np.where(
                        np.logical_not(self.flags[i, j, :, :].T),
                        self.rfi_antenna_data[i, j, :, :].T,
                        0,
                    )
                    ...
                    im1 = ax[0].imshow(
                        self.rfi_antenna_data[i, j, :, :].T, norm=norm1, aspect="auto"
                    )
                    ...
                    fig.suptitle(
                        f"Baseline {i} - Polarization {j} - RFI Percent Flagged: {per:.2f} - Score: {scr:.2f}"
                    )
