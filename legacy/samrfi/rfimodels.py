                ...
                for pol in range(self.RadioRFI.rfi_antenna_data.shape[1]):
                    ...
                    if adding_patch:
                        ...
                        patches, positions = extract_patches(
                            single_data, window_size=256, overlap=128
                        )
                        patches = [patches[i] for i in range(patches.shape[0])]
                    ...
                    for patch in patches:
                        ...
                        inputs = self.processor(
                            single_patch, input_boxes=[[bbox]], logits=True, return_tensors="pt"
                        )
                    ...
                    if adding_patch:
                        ...
                        master_flag = reconstruct_from_patches_adding(
                            cropped_patches, positions, single_data.shape
                        )

                        cropped_patches_prob = crop_patches(np.stack(patch_flags_prob))
                        master_flag_prob = reconstruct_from_patches_adding(
                            cropped_patches_prob, positions, single_data.shape
                        )
                    elif not sliding_patch:
                        master_flag = reconstruct_image(patch_flags, original_shape, padded_shape)
                        master_flag_prob = reconstruct_image(
                            patch_flags_prob, original_shape, padded_shape
                        )
                    elif sliding_patch:
                        master_flag = reconstruct_from_patches(
                            np.stack(patch_flags),
                            positions,
                            single_data.shape,
                            window_size=256,
                            overlap=128,
                        )
                        master_flag_prob = reconstruct_from_patches(
                            np.stack(patch_flags_prob),
                            positions,
                            single_data.shape,
                            window_size=256,
                            overlap=128,
    ...
    def run_model_sam2(
        ...
        for baseline in tqdm(range(self.RadioRFI.rfi_antenna_data.shape[0])):
            ...
            for pol in range(self.RadioRFI.rfi_antenna_data.shape[1]):
                ...
                patches, original_shape, padded_shape = create_patches(
                    single_data, patch_size=patch_size
                )

                if reuse_logits:
                    if counter > 0:
                        logits_reuse, original_shapel, padded_shapel = create_patches(
                            master_logits, patch_size=patch_size
                        )
                ...
                master_flag = reconstruct_image(
                    patch_flags, original_shape, padded_shape, patch_size=patch_size
                )
                master_logits = reconstruct_image(
                    patch_logits, original_shape, padded_shape, patch_size=patch_size
                )
