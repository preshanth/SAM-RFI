...
class Plotter:
    ...
    def plot(self, mode="DATA", baseline=0, polarization=0):
        ...
        if mode == "DATA":
            ...
            ax.imshow(
                data[baseline, polarization, :, :].T, aspect="auto", cmap="viridis", norm=norm
            )
            plt.show()
    ...
    def overplot_detection(self, save=False):
        ...
        masked_spectrograph = np.where(
            np.logical_not(self.RadioRFI.flags), self.RadioRFI.rfi_antenna_data, 0
        )
    ...
    def plotit(
        ...
        ax[1].imshow(
            np.abs(self.RadioRFI.rfi_antenna_data * (1 - self.RadioRFI.flags)), vmin=0, vmax=100
        )
