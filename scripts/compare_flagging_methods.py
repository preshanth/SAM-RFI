...
# CASA imports
try:
    from casatasks import applycal, bandpass, flagdata, gaincal, imstat, tclean
...
def download_tutorial_data(output_dir=".", force=False):
    ...
    if not tarball_path.exists() or force:
        ...
        def progress_hook(block_num, block_size, total_size):
            ...
            if block_num % 100 == 0:
                print(
                    f"  Progress: {percent:.1f}% ({mb_downloaded:.0f}/{mb_total:.0f} MB)",
                    end="\r",
                    flush=True,
                )
        ...
        with tarfile.open(tarball_path, "r:gz") as tar:
            ...
            for i, member in enumerate(members):
                ...
                if i % 1000 == 0:
                    ...
                    print(
                        f"  Progress: {percent:.1f}% ({i}/{total_members} files)",
                        end="\r",
                        flush=True,
                    )
    ...
    print("\nNext steps:")
    print("  1. Process with CASA following the guide")
    print("  2. Run Hanning smoothing to create 3C129_pband.ms")
    print("  3. Use that MS with this comparison script")
    print("=" * 70)
...
class FlaggingComparison:
    ...
    def __init__(
        ...
        # Results storage
        self.results = {"casa": {}, "sam": {}, "comparison": {}}
    ...
    def apply_casa_flagging(self):
        ...
        print(
            f"    After tfcrop: {summary_1['percent']:.2f}% flagged "
        ...
        print("    Computing ionospheric TEC corrections...")
        try:
        ...
        applycal(
            vis=ms, field=self.calibrator_field, applymode="calflagstrict", gaintable=cal_tables
        )
    ...
    def apply_sam_flagging(self):
        ...
        predictor = RFIPredictor(
            model_path=str(self.model_path), sam_checkpoint="large", device="cuda", batch_size=8
        )
    ...
    def create_images(self):
        ...
        # Comparison
        rms_improvement = (
            (casa_metrics["rms_Jy"] - sam_metrics["rms_Jy"]) / casa_metrics["rms_Jy"] * 100
        )
        dr_improvement = (
            (sam_metrics["dynamic_range"] - casa_metrics["dynamic_range"])
            / casa_metrics["dynamic_range"]
            * 100
        )
    ...
    def _plot_image_metrics(self):
        ...
        bars1 = ax1.bar(
            methods, rms_values, color=["#1f77b4", "#ff7f0e"], alpha=0.7, edgecolor="black"
        )

        for bar, val in zip(bars1, rms_values, strict=False):
            ax1.text(
                bar.get_x() + bar.get_width() / 2.0,
                bar.get_height(),
                f"{val:.2f}",
        ...
        bars2 = ax2.bar(
            methods, dr_values, color=["#1f77b4", "#ff7f0e"], alpha=0.7, edgecolor="black"
        )

        for bar, val in zip(bars2, dr_values, strict=False):
            ax2.text(
                bar.get_x() + bar.get_width() / 2.0,
                bar.get_height(),
                f"{val:.1f}",
...
Examples:
  ...
  python compare_flagging_methods.py --ms data.ms --model model.pth --output ./out/ \\
    ...
    # Download mode
    parser.add_argument(
        "--download-data", action="store_true", help="Download VLA P-band tutorial data (26.46 GB)"
    )
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        "--force-download", action="store_true", help="Force re-download even if data exists"
    )
    ...
    if args.download_data:
        ...
        try:
            ms_path = download_tutorial_data(output_dir=args.data_dir, force=args.force_download)
    ...
    if not args.ms or not args.model or not args.output:
        parser.error(
            "--ms, --model, and --output are required for comparison mode. "
