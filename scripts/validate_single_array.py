...
def run_inference(waterfall, config, model_path, sam_checkpoint, device, save_probabilities=None, threshold=None):
    """Run SAM inference."""
    print("\n" + "="*60)
...
def create_plots(waterfall, ground_truth, predicted_mask, probabilities, metrics, output_dir):
    ...
    if ground_truth is not None:
        ...
        im0 = axes[0,0].imshow(magnitude, aspect='auto', cmap='viridis', alpha=0.7,
        ...
        im1 = axes[0,1].imshow(magnitude, aspect='auto', cmap='viridis', alpha=0.7,
                               norm=LogNorm(vmin=vmin, vmax=vmax))
        ...
        im2 = axes[0,2].imshow(magnitude, aspect='auto', cmap='gray', alpha=0.5,
                               norm=LogNorm(vmin=vmin, vmax=vmax))
        axes[0,2].contour(ground_truth, levels=[0.5], colors='red', linewidths=2, alpha=0.9, label='GT')
        axes[0,2].contour(predicted_mask, levels=[0.5], colors='cyan', linewidths=1.5, alpha=0.9, linestyles='--', label='Pred')
        ...
        im3 = axes[1,0].imshow(ground_truth, aspect='auto', cmap='Reds', vmin=0, vmax=1)
        ...
        axes[1,0].text(0.5, 1.05, f'{gt_pct:.2f}% flagged',
                       ha='center', transform=axes[1,0].transAxes, fontsize=11)

        # (1,1) Prediction mask
        im4 = axes[1,1].imshow(predicted_mask, aspect='auto', cmap='Blues', vmin=0, vmax=1)
        ...
        axes[1,1].text(0.5, 1.05, f'{pred_pct:.2f}% flagged',
                       ha='center', transform=axes[1,1].transAxes, fontsize=11)
        ...
        axes[1,3].hist(probabilities.ravel(), bins=100, color='orange', alpha=0.7, edgecolor='black')
        ...
        axes[1,3].axvline(probabilities.mean(), color='red', linestyle='--', linewidth=2, label=f'Mean={probabilities.mean():.3f}')
        ...
        axes[0,1].set_ylabel('Frequency')
        pred_pct = np.sum(predicted_mask) / predicted_mask.size * 100
        axes[0,1].text(0.5, 1.05, f'{pred_pct:.2f}% flagged',
                       ha='center', transform=axes[0,1].transAxes, fontsize=11)
        ...
        axes[1,1].hist(magnitude[~predicted_mask].ravel(), bins=100, alpha=0.7, label='Unflagged', log=True)
        ...
        axes[1,2].hist(probabilities.ravel(), bins=100, color='orange', alpha=0.7, edgecolor='black')
        ...
        axes[1,2].axvline(probabilities.mean(), color='red', linestyle='--', linewidth=2, label=f'Mean={probabilities.mean():.3f}')
...
def main():
    ...
    parser.add_argument('--sam-checkpoint', default='base_plus', help='SAM variant (tiny/small/base_plus/large)')
    parser.add_argument('--device', default='cuda', help='Device (cuda/cpu)')
    parser.add_argument('--output', default='./single_array_validation', help='Output directory')
    parser.add_argument('--input-npy', help='Load real data from .npy file (skips synthetic generation)')
    parser.add_argument('--save-probabilities', help='Save probability maps to .npy file')
    parser.add_argument('--threshold', type=float, default=None, help='RFI probability threshold (default: None=use mean)')
    ...
    # Run SAM inference
    predicted_mask, probabilities = run_inference(waterfall, config, args.model, args.sam_checkpoint, args.device, args.save_probabilities, args.threshold)
